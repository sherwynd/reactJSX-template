...
export function BlogCard({ post }) {
    ...
    const [favouriteCounter, setFavouriteCounter] = useState(post.favouriteCount.length);

    useEffect(() => {
        const fetchCreatorProfile = async () => {
          const response = await fetch(`http://localhost:3000/auth/getAccount/${post.creatorId}`, {
            method: 'GET',
            headers: {
    ...
    return (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', flexGrow: 1 }}>
            avatar={
            <Avatar
            sx={{ bgcolor: red[500], m: 2, justifyContent: 'center', alignItems: 'center' }}
            aria-label="avatar">
            {/*alt={post.name}
        ...
        <Box sx={{ mx: 3, display: 'flex', flexDirection: 'column' }}>
            ...
            <span>
                action={
                <IconButton aria-label="add to favourites" onClick={handleFavourite} style={{ marginRight: '8px' }}>
                {favourite === false ? <FavouriteIcon /> : <FavoriteIcon color="warning" />}
                </IconButton>
                }
